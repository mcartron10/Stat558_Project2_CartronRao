Project 2 F Stat 558 Spoonacular Vignette
================
Cartron and Rao
9/30/2022
...
``` r
recipe_query <- function(key = NULL, query = NULL, cuisine = NULL, diet = NULL, exclude = NULL, number = number, maxReadyTime = NULL, minCalories = NULL, maxCalories = NULL,minCarbs = NULL,maxCarbs = NULL,minProtein = NULL, maxProtein = NULL, minFat = NULL, maxFat = NULL, minSaturatedFat = NULL, maxSaturatedFat = NULL) { 
...
data frame using the aforementioned `recipe_query()` with custom
specified parameters/endpoints. This data frame is then stored in the
object `mydata`. The next step is to loop through all of i recipes,
extract their respective recipe IDs, then, for each recipe, find their
...
Next we perform some data manipulation/cleaning. We use `gsub()` to
remove units from the nutritional information values with `gsub()`, and
...
Below we created three contingency tables (with the function `table()`.
The first two contain the protein content categorical variable and fat
content categorical variable. These are one-way contingency tables,
displaying only the number of recipes that fall into each level of the
two variables (low, medium, or high for each). The third contingency
table combines the two variables, producing a two-way contingency table.
How do we interpret this? For each level of protein or fat, we can look
to see how many recipes fell into the levels of the other variable. For
example, for the level of high protein, there were x recipes with high
fat, y recipes with medium fat, and z recipes with low fat.
...
Note: All plots were created with the function `ggplot()` along with
additional geom layers depending upon the desired type of plot.
...
with, especially as it pertains to ordering their levels in plots. We
did this with the function `factor()`
...
Our first plot is s bar plot that looks at the number of times recipes
...
interesting. It could be that Italian cuisine has more recipes in the
...
of the distribution is `r`mean(anl$calories)\` calories.
...
highest protein category had the highest median amount of protein. The
medium protein category fell between these two categories. As expected,
there was also no overlap between categories.
